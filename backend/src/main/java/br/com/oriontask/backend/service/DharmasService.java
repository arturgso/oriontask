...
public class DharmasService {
    ...
    public DharmasDTO updateDharmas(UpdateDharmasDTO editDTO, Long dharmasId) {
        Dharmas dharmas = repository.findById(dharmasId)
    ...
    public void deleteDharmas(Long dharmasId) {
        ...
        long activeTasksCount = tasksRepository.findByDharmasId(dharmasId, Pageable.unpaged()).stream()
                .filter(task -> task.getStatus() != TaskStatus.DONE)
                .count();

        if (activeTasksCount > 0) {
            throw new IllegalStateException("Cannot delete Dharmas with active tasks. Complete or move tasks first.");
        }
